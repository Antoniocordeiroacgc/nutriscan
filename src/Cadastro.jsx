...
export default function Cadastro({ onCadastrado }) {
  const [etapa, setEtapa]     = useState("inicio"); // inicio | cadastro | dados | objetivo | sucesso | contato | contato_ok | login
  ...
  const enviarContato = async () => {
    ...
    try {
      const { error } = await supabase
        .from("contatos")
        .insert({ nome: cNome, email: cEmail, celular: cCelular, mensagem: cMensagem });
      if (error) throw error;
      setEtapa("contato_ok");
    ...
    <div style={{ minHeight: "100vh", background: "#F7F5F0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px", fontFamily: "system-ui, sans-serif" }}>
      ...
      {etapa !== "contato" && etapa !== "contato_ok" && (
        <div style={{ background: "#1E5C3A", borderRadius: 14, padding: "12px 20px", marginBottom: 20, textAlign: "center", maxWidth: 400, width: "100%" }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)" }}>Faça parte do programa piloto do NutriScan. Sem cartão de crédito.</div>
        </div>
      )}
      ...
      <div className="card" style={{ background: "white", borderRadius: 20, padding: 24, width: "100%", maxWidth: 400, border: "1px solid #F0EFE8" }}>
        ...
        {etapa === "inicio" && (
          <div>
            ...
            <button onClick={() => setEtapa("dados")} style={{ width: "100%", background: "#1E5C3A", color: "white", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer", marginBottom: 8 }}>
              Criar conta grátis →
            </button>
            <button onClick={() => { setEtapa("login"); setErro(""); }} style={{ width: "100%", background: "transparent", color: "#666", border: "1px solid #E8E8E0", borderRadius: 12, padding: 12, fontSize: 13, cursor: "pointer", marginBottom: 8 }}>
              Já tenho conta — Entrar
            </button>
            <button onClick={() => setEtapa("contato")} style={{ width: "100%", background: "transparent", color: "#1E5C3A", border: "1px solid #C8E6D4", borderRadius: 12, padding: 12, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              💬 Fale Conosco
            </button>
          </div>
        )}
        ...
        {etapa === "login" && (
          <div>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Seu e-mail" type="email" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <input value={senha} onChange={e => setSenha(e.target.value)} placeholder="Senha" type="password" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 16, outline: "none", boxSizing: "border-box" }} />
        ...
        {etapa === "dados" && (
          <div>
            ...
            <input value={nome} onChange={e => setNome(e.target.value)} placeholder="Nome completo" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Seu melhor e-mail" type="email" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <input value={senha} onChange={e => setSenha(e.target.value)} placeholder="Crie uma senha" type="password" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 16, outline: "none", boxSizing: "border-box" }} />
            {erro && <div style={{ color: "#C00", fontSize: 12, marginBottom: 10 }}>{erro}</div>}
            <button onClick={() => { if (!nome || !email || !senha) { setErro("Preencha todos os campos."); return; } setErro(""); setEtapa("objetivo"); }} style={{ width: "100%", background: "#1E5C3A", color: "white", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
              Continuar →
            </button>
        ...
        {etapa === "sucesso" && (
          <div style={{ textAlign: "center", padding: "16px 0" }}>
            ...
            <div style={{ fontSize: 13, color: "#888", lineHeight: 1.6 }}>
              Bem-vindo ao NutriScan!<br />Seu período de <strong>30 dias grátis</strong> começou agora.
            </div>
        ...
        {etapa === "contato" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#888", marginBottom: 20, lineHeight: 1.6 }}>
              Tem interesse em usar o NutriScan na sua clínica? Deixe seu contato e retornaremos em breve!
            </div>
            <input value={cNome} onChange={e => setCNome(e.target.value)} placeholder="Seu nome completo" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <input value={cEmail} onChange={e => setCEmail(e.target.value)} placeholder="Seu e-mail" type="email" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <input value={cCelular} onChange={e => setCCelular(e.target.value)} placeholder="WhatsApp / Celular (opcional)" style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
            <textarea value={cMensagem} onChange={e => setCMensagem(e.target.value)} placeholder="Sua mensagem — conte sobre sua clínica ou dúvida..." rows={4} style={{ width: "100%", border: "1px solid #E8E8E0", borderRadius: 10, padding: "10px 12px", fontSize: 14, marginBottom: 16, outline: "none", boxSizing: "border-box", resize: "none", fontFamily: "system-ui, sans-serif" }} />
        ...
        {etapa === "contato_ok" && (
          <div style={{ textAlign: "center", padding: "16px 0" }}>
            ...
            <div style={{ fontSize: 13, color: "#888", lineHeight: 1.6, marginBottom: 20 }}>
              Obrigado pelo interesse! Entraremos em contato em breve pela CRIAR.IA TECNOLOGIA.
            </div>
            <button onClick={() => setEtapa("inicio")} style={{ width: "100%", background: "#1E5C3A", color: "white", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
              Voltar ao início
            </button>
          </div>
        )}
