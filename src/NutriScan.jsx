  ...
  "observacao": "comentário nutricional breve em português"
      ...
      messages: [{
        ...
        content: [
          ...
          { type: "text", text: "Analise este prato e retorne o JSON nutricional completo." }
        ]
      }]
    })
  });
...
async function salvarRefeicaoCompleta(pacienteId, fotoFile, analiseIA) {
  // 1. Converte arquivo para blob jpeg
...
export default function NutriScan({ paciente }) {
  ...
  const analisar = async () => {
    ...
    setResultado(null);
    try {
      const analise = await analisarComIA(base64, "image/jpeg");
  ...
  const enviarParaNutricionista = async () => {
    ...
    setStatus("salvando");
    try {
      const { fotoUrl: url } = await salvarRefeicaoCompleta(PACIENTE_ID, fotoFile, resultado);
      setFotoUrl(url);
      setEnviado(true);
      setStatus("salvo");
    } catch (e) {
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 26, fontWeight: 800, lineHeight: 1 }}>
                {total}<span style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.6)" }}> kcal</span>
              </div>
            </div>
      ...
      <div style={{ padding: "16px" }}>
        ...
        {(status === "analisando" || status === "salvando") && (
          <div style={{ textAlign: "center", padding: "28px 0" }}>
            ...
            <div style={{ fontWeight: 700, color: "#1E5C3A", fontSize: 15 }}>
              {status === "analisando" ? "Analisando o prato com IA..." : "Salvando e enviando foto..."}
            </div>
            <div style={{ fontSize: 12, color: "#aaa", marginTop: 4 }}>
              {status === "analisando" ? "Identificando alimentos e calculando calorias" : "Guardando foto e dados para a nutricionista"}
            </div>
          </div>
        )}
        ...
        {resultado && status !== "analisando" && (
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            ...
            {/* Card total */}
            <div style={{ background: "#1E5C3A", borderRadius: 18, padding: "16px 18px", marginBottom: 12, color: "white" }}>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginBottom: 4 }}>Refeição identificada</div>
              <div style={{ fontWeight: 800, fontSize: 17, marginBottom: 12 }}>{resultado.refeicao}</div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 99, height: 6, width: 100, marginTop: 6, overflow: "hidden" }}>
                    <div style={{ width: `${pct}%`, background: "#7DFCA8", height: "100%", borderRadius: 99 }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
              {resultado.alimentos?.map((a, i) => (
                <div key={i} style={{ background: "white", borderRadius: 14, padding: "11px 14px", display: "flex", alignItems: "center", gap: 12, border: "1px solid #F0EFE8", animation: `fadeUp 0.4s ease ${i * 0.07}s both` }}>
                  <div style={{ width: 44, height: 44, background: "#EEF7F2", borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
                    {emojis[i % emojis.length]}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: "#1a1a1a" }}>{a.nome}</div>
                    <div style={{ fontSize: 11, color: "#aaa", marginTop: 1 }}>{a.porcao} · {a.peso_estimado_g}g</div>
                  </div>
            ...
            {resultado.observacao && (
              <div style={{ background: "#EEF7F2", borderRadius: 13, padding: "11px 14px", display: "flex", gap: 8, marginBottom: 12, alignItems: "flex-start" }}>
                <span style={{ fontSize: 16, flexShrink: 0 }}>💡</span>
                <div style={{ fontSize: 13, color: "#1E5C3A", lineHeight: 1.5 }}>{resultado.observacao}</div>
              </div>
            )}

            {/* Botão enviar / sucesso */}
            {!enviado ? (
              <button
                onClick={enviarParaNutricionista}
                disabled={status === "salvando"}
                style={{ width: "100%", background: "#1E5C3A", color: "white", border: "none", borderRadius: 14, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer", marginBottom: 8 }}
              >
                📤 Salvar foto e enviar para a nutricionista
              </button>
            ) : (
              <div style={{ background: "#EEF7F2", border: "1px solid #C8E6D4", borderRadius: 14, padding: 16, textAlign: "center", animation: "fadeUp 0.4s ease" }}>
                <div style={{ fontSize: 28, marginBottom: 6 }}>✅</div>
                <div style={{ fontWeight: 700, color: "#1E5C3A", fontSize: 15 }}>Foto salva e enviada!</div>
                ...
                <button onClick={reset} style={{ marginTop: 12, background: "transparent", border: "1px solid #C8E6D4", borderRadius: 8, padding: "8px 20px", fontSize: 13, color: "#1E5C3A", cursor: "pointer", fontWeight: 600 }}>
