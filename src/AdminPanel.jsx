...
export default function AdminPanel({ onSair }) {
  ...
  const [aba, setAba]                 = useState("pacientes"); // pacientes | contatos
      ...
      <div style={{ minHeight: "100vh", background: "#0A0A0A", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
        <div style={{ background: "#1a1a1a", borderRadius: 20, padding: 32, width: "100%", maxWidth: 360, border: "0.5px solid #333" }}>
          ...
          <input
            ...
            placeholder="Senha de acesso" type="password"
          ...
          <button onClick={entrar} style={{ width: "100%", background: "#1E5C3A", color: "#7DFCA8", border: "none", borderRadius: 10, padding: 12, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
            Acessar painel
          </button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F5F0", fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "#1E5C3A", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <div>
      ...
      <div style={{ padding: "16px 20px", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          ...
          <button onClick={() => setAba("contatos")} style={{ background: aba === "contatos" ? "#1E5C3A" : "white", color: aba === "contatos" ? "white" : "#666", border: `1px solid ${naoRespondidos > 0 ? "#E24B4A" : "#E8E8E0"}`, borderRadius: 10, padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", position: "relative" }}>
            💬 Contatos ({contatos.length})
            {naoRespondidos > 0 && (
              <span style={{ background: "#E24B4A", color: "white", borderRadius: 99, fontSize: 10, fontWeight: 700, padding: "1px 6px", marginLeft: 6 }}>
                {naoRespondidos} novo{naoRespondidos > 1 ? "s" : ""}
              </span>
            )}
          </button>
          <input
        ...
        {aba === "pacientes" && (
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #F0EFE8", overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1.5fr 1fr 1fr 1fr", padding: "10px 16px", background: "#F7F5F0", borderBottom: "1px solid #F0EFE8" }}>
              {["Nome","E-mail","Objetivo","Refeições","Dias rest.","Status"].map((h, i) => (
            ...
            {carregando ? (
              ...
              <div style={{ textAlign: "center", padding: 32, color: "#aaa" }}>Nenhum paciente encontrado</div>
            ) : pacientesFiltrados.map((p, i) => {
              const dias = diasRestantes(p.trial_fim);
              const refs = refeicoes[p.id] || 0;
              const ativo = dias > 0;
              return (
                <div key={p.id} style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1.5fr 1fr 1fr 1fr", padding: "12px 16px", borderBottom: "1px solid #F7F5F0", background: i % 2 === 0 ? "white" : "#FAFAF8", alignItems: "center" }}>
        ...
        {aba === "contatos" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {contatosFiltrados.length === 0 ? (
              ...
              <div key={c.id} style={{ background: "white", borderRadius: 16, padding: "16px 18px", border: `1px solid ${c.respondido ? "#F0EFE8" : "#FFD0D0"}`, opacity: c.respondido ? 0.7 : 1 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <a href={`mailto:${c.email}`} style={{ background: "#1E5C3A", color: "white", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                      ✉️ Responder
                    </a>
                    {!c.respondido && (
                      <button onClick={() => marcarRespondido(c.id)} style={{ background: "#EEF7F2", color: "#0F6E56", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                        ✓ Marcar respondido
                      </button>
                    )}
                    {c.respondido && (
                      <span style={{ background: "#EEF7F2", color: "#0F6E56", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600 }}>
                        ✓ Respondido
                      </span>
                    )}
                  </div>
                </div>
                <div style={{ background: "#F7F5F0", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#444", lineHeight: 1.6 }}>
                  {c.mensagem}
                </div>
              </div>
            ))}
